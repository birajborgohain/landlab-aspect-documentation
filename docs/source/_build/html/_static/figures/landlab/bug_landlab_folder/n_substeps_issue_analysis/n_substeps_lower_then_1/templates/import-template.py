...
selected_output_dir = (
    "outputs/Landlab_defined_uplift_subset_{n_substeps_time}_trace"
)
...
class MyAspectLandlabModel(LandLabTemplate):
    ...
    def update_until(self,end_time,ASPECT_dim,ASPECT_fields_at_Landlab_nodes_dict):
        ...
        #     n_substeps = {n_substeps_time}
        #     trace(f"n_substeps = {n_substeps_time}")
        #     sub_dt = dt / n_substeps
        ...
        if dt > 0:
            ...
            target_sub_dt = {n_substeps_time}

            n_substeps = max(
                1,
                int(round(dt / target_sub_dt))
            )

            sub_dt = dt / n_substeps

            trace(f"target_sub_dt = {target_sub_dt}")
            ...
            for i in range(n_substeps):
                ...
                report_header(
                    f"LOOP {GLOBAL_LOOP_COUNTER}"
                )

                report(
                    f"elapsed_time_before = "
                    f"{elapsed_time_before}"
                )
                ...
                report(
                f"max deposition_erosion = "
                f"{np.max(deposition_erosion)}"
                )
        ...
        # report_header("FINAL SUMMARY")
        report_header(
            f"ASPECT TIMESTEP {self.timestep} SUMMARY"
        )

        report(
            f"Total loops = {n_substeps_time}"
        )

        report(
            f"Final elapsed_time = "
            f"{self.current_time}"
        )

        report(
            f"Final max elevation = "
            f"{np.max(self.elevation)}"
        )
        return dimensional_deposition_erosion
