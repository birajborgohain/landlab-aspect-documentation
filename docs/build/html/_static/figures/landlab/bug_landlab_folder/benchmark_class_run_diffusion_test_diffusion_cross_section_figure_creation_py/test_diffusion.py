...
def test_diffusion():
    ...
    with open(TRACE_FILE, "w") as f:
        ...
        f.write(
            "Standalone Landlab Execution Trace\n"
        )
    ...
    while elapsed_time < time_to_run:
        ...
        if elapsed_time + current_dt > time_to_run:
            ...
            trace(
                f"Adjusted final dt = {current_dt}"
            )

        trace(
            f"Using dt = {current_dt}"
        )
        ...
        trace(
            f"Updating elapsed_time "
            f"from {elapsed_time} "
            f"to {elapsed_time + current_dt}"
        )

        elapsed_time += current_dt

        trace(
            f"elapsed_time_after = {elapsed_time}"
        )
