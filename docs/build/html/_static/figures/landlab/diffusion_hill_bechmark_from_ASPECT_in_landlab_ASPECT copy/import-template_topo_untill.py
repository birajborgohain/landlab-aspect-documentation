...
class MyAspectLandlabModel(LandLabTemplate):
    ...
    def set_mesh_information(self, grid_dictionary):
        ...
        # # #RasterModelGrid + one ghost nodes on each side
        ...
        # approximate counts
        ncols = int(x_extent / dx) + 1

        # hex vertical spacing
        dy = np.sqrt(3.0) / 2.0 * dx
        nrows = int(y_extent / dy) + 1

        self.model_grid = landlab.HexModelGrid(
            (nrows, ncols),
            # xy_spacing=(spacing, spacing),
            spacing=dx,
        ...
        print(self.model_grid.node_x.min())
        print(self.model_grid.node_x.max())
        print(self.model_grid.node_x[:20])

        import sys

        print("\n========== HEX GRID DEBUG ==========")
        print(self.model_grid.node_x[:50])
        print(self.model_grid.node_y[:50])
        print("====================================\n")


        # # keep only nodes inside ASPECT domain
        # inside = (
        #     (self.model_grid.x_of_node >= 0.0) &
        #     (self.model_grid.x_of_node <= x_extent) &
        #     (self.model_grid.y_of_node >= 0.0) &
        #     (self.model_grid.y_of_node <= y_extent)
        # )

        # print("x range:",
        #     self.model_grid.x_of_node[inside].min(),
        #     self.model_grid.x_of_node[inside].max())

        # print("y range:",
        #     self.model_grid.y_of_node[inside].min(),
        #     self.model_grid.y_of_node[inside].max())
        ...
        x = self.model_grid.node_x.copy()

        row_ids = np.round(
            self.model_grid.node_y / (np.sqrt(3)/2 * dx)
        ).astype(int)

        # remove half-cell shift from alternating rows
        x[row_ids % 2 == 1] -= dx / 2
        ...
        # # only apply sinusoid to top boundary nodes
        # top_nodes = self.model_grid.nodes_at_top_edge

        # x_top = self.model_grid.x_of_node[top_nodes]

        # z[top_nodes] = A * np.sin(np.pi * x_top / L)
        ...
        self.model_grid.status_at_node[
            self.model_grid.boundary_nodes
        ] = NodeStatus.CLOSED
