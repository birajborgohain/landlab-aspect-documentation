...
def get_aspect_output_directory():
    ...
    aspect_output_directories = []

    for directory_name in os.listdir(outputs_root):
        ...
        if directory_name.lower().startswith("default"):
            continue

        solution_directory = os.path.join(
            full_path,
            "solution"
        )
        ...
        aspect_output_directories.append(
            full_path
        )

    if len(aspect_output_directories) == 0:

        raise RuntimeError(
            "No valid ASPECT output directories found."
        )

    aspect_output_directories.sort(
        key=os.path.getmtime,
        reverse=True
    )

    return aspect_output_directories[0]
...
class MyAspectLandlabModel(LandLabTemplate):
    ...
    def update_until(self,end_time,ASPECT_dim,ASPECT_fields_at_Landlab_nodes_dict):
        ...
        if dt > 0:
            ...
            # n_substeps = 1.5
            desired_sub_dt = 0.6666667
            n_substeps = int(np.ceil(dt / desired_sub_dt))
    ...
    def set_mesh_information(self, grid_dictionary):
        ...
        trace(
            f"TRACE INITIALIZED | output_dir={selected_output_dir}"
        )
